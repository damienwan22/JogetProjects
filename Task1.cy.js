...
describe('Joget Cloud Partners Page', () => {
  it('Successfully loads and displays correctly', () => {
    ...
    cy.title().then((title) => {
      expect(title).to.include('Joget Cloud');
      expect(title).to.include('Partners');
    })

    // Check if there is at least one body element
    cy.get('body').should('be.visible')

    // Test 3: Check if all images are loaded.
    cy.get('img').each(($img) => {
      cy.wrap($img).should('be.visible') // This will check if the image is visible, not necessarily if it's fully loaded
    })
  })
})
...
describe('Form Submission', () => {
    it('Cannot be submitted until all required fields are filled.', () => {
      ...
      //Verify that the button would become enabled after fields are filled
      cy.xpath("//span[normalize-space()='Submit']").should('be.enabled')
...
describe('Verifying if the form could be Submitted', () => {
  it('Must have Submit button working.', () => {
    ...
    // Verifying that the form should be enabled
    cy.xpath("//span[normalize-space()='Submit']").should('be.enabled')
    cy.xpath("//span[normalize-space()='Submit']").click()

    cy.xpath("//span[normalize-space()='Start here']").should('be.enabled')
    cy.xpath("//span[normalize-space()='Start here']").click()
  })
})
...
describe('Clickable Buttons and Correct Displays', () => {
  it('Must have all the links and buttons clickable and open the correct page', () => {
    ...
    // Getting both the buttons to check that they are functional and not disabled
    cy.get('[style="text-align:left; margin-top:10px; margin-bottom:10px;"] > .wsite-button > .wsite-button-inner').each(($btn) => {
      cy.wrap($btn).should('not.be.disabled');
    });
    
    cy.get('[style="width:33.307984790875%; padding:0 15px;"] > :nth-child(3) > .wsite-button > .wsite-button-inner').each(($btn) => {
      cy.wrap($btn).should('not.be.disabled');
    });
